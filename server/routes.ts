...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/study-plans/:userId", async (req, res) => {
    try {
      ...
      const studyPlans = await storage.getStudyPlans(userId);
      res.json(studyPlans);
    } catch (error) {
  ...
  app.get("/api/study-plan/:id", async (req, res) => {
    try {
      if (!req.isAuthenticated()) return res.sendStatus(401);
      ...
      if (req.user!.id !== studyPlan.userId && req.user!.role !== 'admin') {
        return res.status(403).json({ error: "Access denied" });
      }
      res.json(studyPlan);
    } catch (error) {
  ...
  app.post("/api/study-plan", async (req, res) => {
    try {
      ...
      const studyPlan = await storage.createStudyPlan(req.body);
  ...
  // Study Plan Items routes
  app.get("/api/study-plan/:planId/items", async (req, res) => {
    try {
      if (!req.isAuthenticated()) return res.sendStatus(401);
      ...
      const items = await storage.getStudyPlanItems(planId);
      res.json(items);
    } catch (error) {
      res.status(500).json({ error: (error as Error).message });
    }
  ...
  app.post("/api/study-plan-item", async (req, res) => {
    try {
      if (!req.isAuthenticated()) return res.sendStatus(401);
      ...
      const item = await storage.createStudyPlanItem(req.body);
  ...
  app.put("/api/study-plan-item/:id", async (req, res) => {
    try {
      if (!req.isAuthenticated()) return res.sendStatus(401);
      const itemId = parseInt(req.params.id);
      const item = await storage.getStudyPlanItem(itemId);
      if (!item) {
        return res.status(404).json({ error: "Study plan item not found" });
      }
      
      const studyPlan = await storage.getStudyPlan(item.planId);
      if (!studyPlan) {
        return res.status(404).json({ error: "Study plan not found" });
      }
      ...
      const updatedItem = await storage.updateStudyPlanItem(itemId, req.body);
  ...
  app.delete("/api/study-plan-item/:id", async (req, res) => {
    try {
      if (!req.isAuthenticated()) return res.sendStatus(401);
      const itemId = parseInt(req.params.id);
      const item = await storage.getStudyPlanItem(itemId);
      ...
      await storage.deleteStudyPlanItem(itemId);
      res.sendStatus(204);
    } catch (error) {
      res.status(500).json({ error: (error as Error).message });
    }
  ...
  app.post("/api/generate-study-plan", async (req, res) => {
    try {
      ...
      const options = {
        ...req.body.options,
        startDate: new Date(req.body.options.startDate),
        endDate: new Date(req.body.options.endDate),
        targetExamDate: req.body.options.targetExamDate ? new Date(req.body.options.targetExamDate) : undefined
      };
      
      const studyPlan = await storage.generateStudyPlan(userId, options);
      res.status(201).json(studyPlan);
    } catch (error) {
      res.status(500).json({ error: (error as Error).message });
    }
